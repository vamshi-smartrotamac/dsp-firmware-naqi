...
// MSDK Debug flags
// Gated on RTOS_RUNTIME_STATS (defined in project.mk for debug builds only),
// NOT on DEBUG directly. This keeps the reference here and the definition in
// FreeRTOS_Debug.c bound to a single macro so they can never diverge, and makes
// the stats timer vanish in release / DEBUG=0 builds. See project.mk.
#if defined(RTOS_RUNTIME_STATS)
