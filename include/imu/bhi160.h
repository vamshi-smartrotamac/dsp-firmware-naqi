...
#define BHY_I2C_REG_BUFFER_LENGTH                                              \
  ((BHY_I2C_REG_BUFFER_END_ADDR) - (BHY_I2C_REG_BUFFER_ZERO_ADDR) + 1)
...
#define BHY_WR_FUNC_PTR                                                        \
  s8 (*bus_write)(mxc_i2c_regs_t *, uint8_t, uint8_t, uint8_t *, uint16_t)
#define BHY_RD_FUNC_PTR                                                        \
  s8 (*bus_read)(mxc_i2c_regs_t *, uint8_t, uint8_t, uint8_t *, uint16_t)
#define BHY_BRD_FUNC_PTR                                                       \
  s8 (*burst_read)(mxc_i2c_regs_t *, uint8_t, uint8_t, uint8_t *, uint32_t)

#define BHY_BUS_READ_FUNC(inst, device_addr, reg_addr, reg_data, r_len)        \
  bus_read(inst, device_addr, reg_addr, reg_data, r_len)

#define BHY_BURST_READ_FUNC(inst, device_addr, register_addr, register_data,   \
                            rd_len)                                            \
...
struct accel_physical_status_t {
  ...
  uint16_t
      accel_dynamic_range; /**<contains the accel dynamic range information*/
...
struct sensor_configuration_non_wakeup_t {
  ...
  uint16_t
      non_wakeup_max_report_latency; /**<contains the maximum report latency*/
...
int8_t bhy_mapping_matrix_set(mxc_i2c_regs_t *i2c_inst,
                              bhy_physical_sensor_index_type_t index,
